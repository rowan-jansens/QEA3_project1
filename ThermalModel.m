...
tic;
t_span = [0.1:1:2000]; % t is in seconds, give it 25 days
...
k_matrix(:,1:2*bread_thikness) = k_bread;
k_matrix(1:bread_thikness,:) = k_bread;

k_matrix(:,end-2*bread_thikness+1:end) = k_bread;
...
[t, temp_tensor, cook_time] = ode45(@simulate_temp, t_span, temps_init, options);
